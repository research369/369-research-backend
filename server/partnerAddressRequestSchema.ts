...
/**
 * Additive, idempotent schema for auditable partner delivery-address requests.
 * Partner portal submissions are never applied to the canonical partner address
 * until an authenticated WaWi administrator explicitly approves the request.
 */
export async function ensurePartnerAddressRequestSchema(): Promise<void> {
  const pool = await getPool();
  if (!pool) throw new Error("Datenbankverbindung für Partneradressanträge nicht verfügbar");
  ...
  console.log("[Partner Address Requests] Schema und zentrale Benachrichtigungseinstellungen bereit");
}
