...
interface OrderEmailData {
  orderId: string;
  storeKey?: "369research" | "peps4pets" | "checkout-v2";
...
function getPaymentMethodLabel(method: string): string {
  switch (method) {
    ...
    case "wise": return "SEPA- oder Echtzeitüberweisung (Wise)";
    ...
    <div style="background:#ffffff;padding:24px;border:1px solid #e5e7eb;">
      ...
      <div style="border-top:2px solid #e5e7eb;padding-top:12px;">
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tr><td style="padding:4px 0;font-size:14px;color:#6b7280;">Versand${data.items.some(i => (i as any).isPlugPlay || ((i as any).isNasalSpray && !(i as any).isNasalDiySet)) ? ' \u2744\ufe0f' : ''}</td><td style="padding:4px 0;font-size:14px;text-align:right;">${data.shipping > 0 ? data.shipping.toFixed(2) + " €" : "Kostenlos"}</td></tr>
          ${data.items.some(i => (i as any).isPlugPlay || ((i as any).isNasalSpray && !(i as any).isNasalDiySet)) ? '<tr><td colspan="2" style="padding:2px 0 6px;font-size:12px;color:#0284c7;">\u2744\ufe0f Inkl. Aufpreis f\u00fcr gek\u00fchlten Versand (+7 \u20ac) aufgrund aufwendiger K\u00fchlverpackung</td></tr>' : ''}
          ${data.items.some(i => (i as any).isPlugPlay || ((i as any).isNasalSpray && !(i as any).isNasalDiySet)) ? `<tr><td colspan="2" style="padding:6px 0 4px;"><div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:12px 14px;font-size:13px;color:#9a3412;"><strong>\u26a0\ufe0f Wichtiger Versandhinweis (K\u00fchlprodukte)</strong><br>Bestellungen mit Patronen (Plug &amp; Play) oder Nasensprays werden aus Gr\u00fcnden der K\u00fchlkettensicherheit ausschlie\u00dflich <strong>Montag bis Donnerstag</strong> versendet. Bestellungen die nach Donnerstag eingehen, werden am darauffolgenden <strong>Montag</strong> verschickt.</div></td></tr>` : ''}
  ...
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    ...
    <div style="background:#fff;padding:24px;border:1px solid #e5e7eb;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr><td style="padding:3px 0;font-size:13px;color:#6b7280;">Versand${data.items.some(i => (i as any).isPlugPlay || ((i as any).isNasalSpray && !(i as any).isNasalDiySet)) ? ' \u2744\ufe0f' : ''}</td><td style="padding:3px 0;font-size:13px;text-align:right;">${data.shipping > 0 ? data.shipping.toFixed(2) + " €" : "Kostenlos"}</td></tr>
        ${data.items.some(i => (i as any).isPlugPlay || ((i as any).isNasalSpray && !(i as any).isNasalDiySet)) ? '<tr><td colspan="2" style="padding:1px 0 4px;font-size:11px;color:#0284c7;">\u2744\ufe0f Inkl. Aufpreis f\u00fcr gek\u00fchlten Versand (+7 \u20ac)</td></tr>' : ''}
